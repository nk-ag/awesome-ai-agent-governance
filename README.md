...
- [Helio](https://github.com/gethelio/helio) - Apache-2.0 governance proxy for MCP agents. Every tool call passes a policy engine, evidence grounding checks, approval workflows, and cross-tool spend budgets that deplete one shared pot across several tools and servers, then lands in an audit trail. Runs in front of unmodified agents and MCP servers; the runtime and dashboard ship in one package (`npx @gethelio/proxy init`).
...
- [Kepil MCP server](https://github.com/oleg-vdv/kepil) - Seven tools for creating orders, running steps, asking whether an action is permitted by the mandate, and verifying the journal. Confirming an irreversible action is deliberately not among them: a model that could approve itself would make the rest decoration, and a test fails if a confirm tool ever appears in the list.
